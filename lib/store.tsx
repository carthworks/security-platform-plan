...
import { ALL_SPRINTS } from './sprints-data';
...
export const DEFAULT_SETTINGS: PlatformSettings = {
  platformName: 'PostureIntel',
  orgName: 'Security Ops Team',
  subdomain: 'postureintel',
  themeColor: '#1D9E75',
  nvdApiKey: '',
  claudeApiKey: 'sk-ant-*******',
  jiraBaseUrl: 'https://securityplatform.atlassian.net',
  jiraProjectKey: 'SEC',
  jiraEmail: 'security@securityplatform.in',
  jiraApiToken: '********',
  slackWebhookUrl: 'https://hooks.slack.com/services/****',
  slackChannel: '#alerts',
  slackCriticalOnly: true,
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const getOverallProgress = useCallback(() => {
    ...
    const staticTasksCount = ALL_SPRINTS.reduce((sum, s) => sum + s.tasks.length, 0);
    const totalTasks = staticTasksCount + totalCustomTasks;
    return { totalDone, totalTasks, pct: totalTasks > 0 ? Math.round((totalDone / totalTasks) * 100) : 0 };
